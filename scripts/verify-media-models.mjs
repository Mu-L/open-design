...
// (src/media/models.ts) and the JS mirror used by the Node daemon
// (daemon/media-models.js). The two are kept in sync by hand because the
...
const TS_PATH = path.join(ROOT, 'src', 'media', 'models.ts');
const JS_PATH = path.join(ROOT, 'daemon', 'media-models.js');
...
if (diffs.length > 0) {
  process.stderr.write(
    'verify-media-models: drift detected between src/media/models.ts and daemon/media-models.js\n',
  );
